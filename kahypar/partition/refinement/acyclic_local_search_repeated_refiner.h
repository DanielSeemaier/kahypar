...
class AcyclicLocalSearchRepeatedRefiner final : public IRefiner {
 ...
 public:
  ...
  void printSummary() const override {
    ...
    LOG << "[AcyclicLocalSearchRepeatedRefiner] Improved KM1 by:" << _improved_km1;
    LOG << "[AcyclicLocalSearchRepeatedRefiner] Total size of initial PQs:" << _initial_pq_size;
    LOG << "[AcyclicLocalSearchRepeatedRefiner] Number of initially enabled PQs:" << _num_initially_enabled_pqs;
    LOG << "[AcyclicLocalSearchRepeatedRefiner] Max part weights:" << _context.partition.max_part_weights[0];
    LOG << "[AcyclicLocalSearchRepeatedRefiner] Perform moves time:" << _perform_moves_time;
    LOG << "[AcyclicLocalSearchRepeatedRefiner] Refine time:" << _refine_time;
    LOG << "[AcyclicLocalSearchRepeatedRefiner] Refine until best cut time:" << _actual_refine_time;
 ...
 private:
  ...
  bool refineImpl(std::vector<HypernodeID>& refinement_nodes,
    ...
    while (!_pq.empty() && !_stopping_policy.searchShouldStop(touched_hns_since_last_improvement,
      ...
      const bool move_ok = weight_ok && acyclicity_ok;
      ...
      if (acyclicity_ok) {
        ...
        for (const PartitionID &part : _gain_manager.adjacentParts(max_gain_node)) {
          ...
          if (!isLockedTarget(max_gain_node, part)) {
            ASSERT(_pq.contains(max_gain_node, part));
        ...
        lockTarget(max_gain_node, to_part);
        if (!_pq.contains(max_gain_node)) {
          _hg.mark(max_gain_node);
        }
      }

      DBG << "Move[" << move_ok << "]" << max_gain_node << "/" << from_part << "-->" << to_part << "/"
          << max_gain << "/ Weights OK:" << weight_ok << "/ Acyclicity OK:" << acyclicity_ok << "/ Move OK:" << move_ok;
  ...
  bool isLockedTarget(const HypernodeID hn, const PartitionID target) const {
    return _locked_target[hn][target];
  }

  void lockTarget(const HypernodeID hn, const PartitionID target) {
    if (!isLockedTarget(hn, target)) {
      _locked_target[hn][target] = true;
      _locked_target_entries.emplace_back(hn, target);
    }
  }
